...
export function ChessNotation({ moves, startFen }: ChessNotationProps) {
  if (!moves || moves.length === 0)
    return <span className="italic text-xs">Ходів немає</span>;

  const isBlackFirst = startFen ? startFen.split(' ')[1] === 'b' : false;
  const elements: React.ReactNode[] = [];

  if (isBlackFirst) {
    elements.push(<span key="first-black">1. ... {moves[0]}</span>);
    ...
    for (let i = 1; i < moves.length; i += 2) {
      ...
      elements.push(
        <span key={`group-${i}`}>
          ...
          {blackMove && <span className="ml-1">{blackMove}</span>}
        </span>,
      );
    }
  } else {
    for (let i = 0; i < moves.length; i += 2) {
      ...
      elements.push(
        <span key={`group-${i}`} className="mr-1">
          {moveNumber}.<span>{whiteMove}</span>
          {blackMove && <span className="ml-1">{blackMove}</span>}
        </span>,
      );
    }
